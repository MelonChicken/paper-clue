...
`.trim();
  ...
  try {
    ...
    const processedEvidence = (parsed.evidence || []).map((e: any) => {
      ...
      const claimStr = e.claimText || "?ºÎ¨∏ Î©îÌ??∞Ïù¥??Í≤ÄÏ¶??ÑÎ£å";
      const srcTitle = e.sourceReference || (srcType === "PAPER" ? "?ºÎ¨∏ ?êÎ¨∏" : "Í≥µÏãù ?ôÏà† Ï∂úÏ≤ò");
      const srcLoc = e.evidenceLocation || (srcType === "PAPER" ? "Abstract / Section 1" : "Í≥µÏãù ?ôÏà† ?πÏÇ¨?¥Ìä∏");
    ...
    if (processedEvidence.length === 0 && finalIdentityStatus !== "IDENTITY_NOT_FOUND") {
      processedEvidence.push({
        ...
        sourceUrl: finalUrl,
        sourceLocation: "?êÎ¨∏ ?úÏ? ?ïÎ≥¥",
        evidenceLocation: "?êÎ¨∏ ?úÏ? ?ïÎ≥¥",
        claim: `Í≥µÏãù ?ºÎ¨∏ ?úÏ? ?ùÎ≥Ñ ?ÑÎ£å: ${finalCanonicalTitle} (${finalAuthors.slice(0, 2).join(", ")} et al., ${finalYear})`,
        claimText: `Í≥µÏãù ?ºÎ¨∏ ?úÏ? ?ùÎ≥Ñ ?ÑÎ£å: ${finalCanonicalTitle} (${finalAuthors.slice(0, 2).join(", ")} et al., ${finalYear})`,
    ...
    const verifiedResult: VerifiedMetadataResult = {
      ...
      publishingReliabilityDetails: {
        ...
        scoreReason:
          ...
          `${finalVenue} Î©îÌ??∞Ïù¥??Î∞?${finalIdentityStatus} ?†Ïõê Í≤ÄÏ¶??ÑÎ£å`,
      ...
      evidence: [
        {
          ...
          sourceLocation: "?ÑÎ≥¥ ?ùÎ≥Ñ ?ïÎ≥¥",
          evidenceLocation: "?ÑÎ≥¥ ?ùÎ≥Ñ ?ïÎ≥¥",
